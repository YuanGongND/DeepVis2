...
#%%
def soundNet( input, numClass = 2, activationUnit = 'relu', l2_reg = 0.01, init = 'lecun_uniform', biasInit = 'Zeros', denseUnitNum = 64, denseUnit = 'relu', dropoutRate = 0.5, timeStep_num = 160 ):
    
    # conv1 pool1
    input = tf.convert_to_tensor( input, name = 'networkInput' )
    example_num = input.get_shape().as_list()[ 0 ]
    example_length = input.get_shape().as_list()[ 0 ]
    input = tf.reshape( input, [ example_num, 1, example_length, 1 ] )
